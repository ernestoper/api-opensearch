...
# app = FastAPI(
#     title="Deviations API",
#     description="API for querying camera deviations stored in OpenSearch",
#     version="1.0.0"
# )
...
# # Endpoint to query deviations
# @app.get("/deviations", response_model=Dict[str, List[Dict[str, Any]]])
# def get_deviations(
#     camera_name: Optional[str] = Query(None, description="Filtrar por nome da câmera"),
#     event_type: Optional[str] = Query(None, description="Filtrar por tipo de evento"),
#     camera_type: Optional[str] = Query(None, description="Filtrar por tipo de câmera"),
...
#         # Add filters to the query
#         if camera_name:
#             query["query"]["bool"]["must"].append({"term": {"camera_name": camera_name}})
        
#         if event_type:
#             query["query"]["bool"]["must"].append({"term": {"event_type": event_type}})
            
#         if camera_type:
#             query["query"]["bool"]["must"].append({"term": {"camera_type": camera_type}})
...
# # Endpoint to query a deviation by ID
# @app.get("/deviations/{deviation_id}", response_model=Dict[str, Any])
# def get_deviation_by_id(
#     deviation_id: str,
#     opensearch_client: OpenSearch = Depends(get_opensearch_client)
# ):
...
# # Class to save deviations to OpenSearch
# class DeviationSaver:
#     def __init__(self, opensearch_client: OpenSearch):
#         self.opensearch_client = opensearch_client
    
#     def save_deviation_to_opensearch(self, deviation_data: dict):
#         """Salva os dados do desvio no OpenSearch."""
#         try:
#             index_name = "deviations"
...
#                                 "camera_type": {"type": "keyword"},  # Changed to keyword for exact matching
#                                 "timestamp": {"type": "date"},
#                                 "crop_video_source": {"type": "text"},
#                                 "event_type": {"type": "keyword"},  # Changed to keyword for exact matching
...
#             document = {
#                 "camera_name": deviation_data['camera_name'],
#                 "camera_type": deviation_data['camera_type'],
#                 "timestamp": deviation_data['timestamp'],
#                 "crop_video_source": deviation_data['crop_video_source'],
#                 "event_type": deviation_data['event_type'],
#                 "event_description": deviation_data['event_description']
#             }
...
#                 print(f"[OPENSEARCH] Desvio salvo com sucesso: {deviation_data['crop_video_source']}")
#                 return {"status": "success", "id": response['_id']}
#             else:
#                 print(f"[OPENSEARCH] Erro ao salvar desvio: {response}")
#                 return {"status": "error", "message": response}
#         except Exception as e:
#             error_msg = f"Erro ao salvar no OpenSearch: {str(e)}"
#             print(error_msg)
#             return {"status": "error", "message": error_msg}

# # Endpoint to save a new deviation
# @app.post("/deviations", response_model=Dict[str, Any])
# async def save_deviation(
#     deviation_data: dict,
#     opensearch_client: OpenSearch = Depends(get_opensearch_client)
# ):
#     saver = DeviationSaver(opensearch_client)
#     result = saver.save_deviation_to_opensearch(deviation_data)
...
# # Verifica se os arquivos config.ini e .env existem
# if not config_file.exists():
#     raise FileNotFoundError("Arquivo config.ini não encontrado!")
# if not env_file.exists():
#     raise FileNotFoundError("Arquivo .env não encontrado!")
...
# # Verifica se o cliente OpenSearch está conectado
# if not opensearch_client.ping():
#     raise Exception("Não foi possível conectar ao OpenSearch!")

# # Endpoint para consultar desvios
# @app.get("/deviations")
# def get_deviations(
#     camera_name: Optional[str] = Query(None, description="Filtrar por nome da câmera"),
#     event_type: Optional[str] = Query(None, description="Filtrar por tipo de evento"),
...
# ):
#     try:
#         # Construa a query do OpenSearch com base nos filtros
#         query = {"query": {"bool": {"must": []}}}

#         if camera_name:
#             query["query"]["bool"]["must"].append({"term": {"camera_name.keyword": camera_name}})
#         if event_type:
#             query["query"]["bool"]["must"].append({"term": {"event_type.keyword": event_type}})
#         if start_time and end_time:
#             query["query"]["bool"]["must"].append({
#                 "range": {
#                     "timestamp": {
...
#         # Execute a consulta no OpenSearch
#         response = opensearch_client.search(
#             index="deviations",
#             body=query
#         )
...
# # Endpoint para consultar um desvio por ID
# @app.get("/deviations/{deviation_id}")
# def get_deviation_by_id(deviation_id: str):
#     try:
#         response = opensearch_client.get(
#             index="deviations",
#             id=deviation_id
